...
function MoleBoard({ seniorMode, onComplete, onSkip, onResult }: Props & { onResult: (score: number) => void }) {
        ...
        {moles.map((active,i) => (
          <button key={i} onPointerDown={() => hitMole(i)}
            className={`aspect-square rounded-2xl border-2 transition-all active:scale-95 flex items-center justify-center
              ${active?"border-[#b89a5a] bg-[#b89a5a]/20 scale-105":"border-[#2a2924] bg-[#18181a]"}`}>
...
export default function MoleGame({ seniorMode, onComplete, onSkip }: Props) {
  ...
  if (result !== null) {
    const passed = result >= PASS_SCORE;
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-6 px-4">
        ...
        <p className={`text-[#c4bfb4] ${seniorMode?"text-xl":"text-base"}`}>{result}마리 (목표: {PASS_SCORE}마리)</p>
  ...
  return <MoleBoard key={key} seniorMode={seniorMode} onComplete={onComplete} onSkip={onSkip} onResult={setResult}/>;
}
