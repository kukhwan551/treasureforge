...
export type GameType = "mole" | "tetris" | "brick" | "memory" | "snake";
...
export const GAME_LABELS: Record<GameType, { name: string; icon: string; desc: string }> = {
  ...
  tetris: { name: "테트리스",    icon: "🟦", desc: "블록을 쌓아 목표 줄 수를 지우면 통과!" },
  brick:  { name: "벽돌깨기",   icon: "🧱", desc: "공으로 벽돌을 모두 깨면 통과!" },
  memory: { name: "기억력 게임", icon: "🃏", desc: "카드를 모두 맞히면 통과!" },
  snake:  { name: "스네이크",   icon: "🐍", desc: "먹이를 목표 수만큼 먹으면 통과!" },
};
...
export default function GamePopup({ gameType, target, postName, seniorMode, onComplete, onSkip }: GamePopupProps) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0f0f10]/95 px-4">
        <div className="w-full max-w-sm text-center space-y-6">
          ...
          <div className="rounded-2xl border border-[#2a2924] bg-[#18181a] px-5 py-4 space-y-1">
            <p className={`text-[#c4bfb4] ${seniorMode?"text-xl":"text-base"}`}>{info.desc}</p>
            {target && gameType !== "memory" && (
              <p className={`text-[#b89a5a] font-bold ${seniorMode?"text-lg":"text-sm"}`}>
                🎯 목표: {gameType==="mole"?`${target}마리`:gameType==="tetris"?`${target}줄`:gameType==="snake"?`${target}개`:target}
              </p>
            )}
          </div>
  ...
  const props = { seniorMode, target, onComplete, onSkip };
  return (
    <div className="fixed inset-0 z-50 bg-[#0f0f10]">
      ...
      {gameType === "snake"  && <SnakeGame  {...props}/>}
    </div>
  );
}
