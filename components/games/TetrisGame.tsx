...
const COLS = 10, ROWS = 18, PASS_LINES = 10;
...
function TetrisBoard({ seniorMode, onComplete, onSkip, onResult }: Props & { onResult: (lines: number, score: number) => void }) {
  ...
  const lock = useCallback(() => {
    const {shape,color} = piece.current;
  ...
  useEffect(() => {
    ...
    function loop(ts:number) {
      ...
      draw();
      if(alive.current) requestAnimationFrame(loop);
    }
...
export default function TetrisGame({ seniorMode, onComplete, onSkip }: Props) {
  ...
  if (result !== null) {
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-6 px-4">
        ...
        <p className={`text-[#c4bfb4] ${seniorMode?"text-xl":"text-base"}`}>{result.lines}줄 (목표: {PASS_LINES}줄)</p>
  ...
  return <TetrisBoard key={key} seniorMode={seniorMode} onComplete={onComplete} onSkip={onSkip} onResult={(l,s)=>setResult({lines:l,score:s})}/>;
}
