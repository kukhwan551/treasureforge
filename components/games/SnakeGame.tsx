...
function SnakeBoard({seniorMode,target,onComplete,onSkip,onDead}:Props&{onDead:(eaten:number)=>void}){
  const PASS=target||5;
  const canvasRef=useRef<HTMLCanvasElement>(null);
  const s=useRef({snake:[{x:8,y:10},{x:7,y:10},{x:6,y:10}] as Pt[],dir:"R" as Dir,nextDir:"R" as Dir,food:{x:12,y:10} as Pt,eaten:0});
  ...
  useEffect(()=>{
    ...
    const W=cvs.width=COLS*CELL,H=cvs.height=ROWS*CELL;
    const SPEED=seniorMode?250:180;
    function randFood(snake:Pt[]):Pt{let pt:Pt;do{pt={x:Math.floor(Math.random()*COLS),y:Math.floor(Math.random()*ROWS)};}while(snake.some(p=>p.x===pt.x&&p.y===pt.y));return pt;}
    function draw(){
      ...
      ctx.fillStyle="#ff4444";ctx.beginPath();ctx.arc(s.current.food.x*CELL+CELL/2,s.current.food.y*CELL+CELL/2,CELL/2-2,0,Math.PI*2);ctx.fill();
    }
    function loop(ts:number){
      if(ts-lastRef.current>SPEED){
        lastRef.current=ts;
        const cur=s.current;cur.dir=cur.nextDir;
        const head={...cur.snake[0]};
        if(cur.dir==="R")head.x++;else if(cur.dir==="L")head.x--;else if(cur.dir==="U")head.y--;else head.y++;
        if(head.x<0||head.x>=COLS||head.y<0||head.y>=ROWS||cur.snake.some(p=>p.x===head.x&&p.y===head.y)){draw();onDead(cur.eaten);return;}
        cur.snake.unshift(head);
        if(head.x===cur.food.x&&head.y===cur.food.y){
          cur.eaten++;setEaten(cur.eaten);
          if(cur.eaten>=PASS){draw();onComplete(cur.eaten*20);return;}
  ...
  },[seniorMode,onComplete,onDead,PASS]);

  function setDir(d:Dir){const cur=s.current.dir;if((d==="U"&&cur==="D")||(d==="D"&&cur==="U")||(d==="L"&&cur==="R")||(d==="R"&&cur==="L"))return;s.current.nextDir=d;}
  ...
  return(
    <div className="h-full flex flex-col items-center py-3 space-y-2">
      <div className="flex items-center gap-6">
        <p className={`text-[#4a9d6f] font-bold ${seniorMode?"text-xl":"text-base"}`}>🍎 {eaten}/{PASS}</p>
        <p className={`text-[#7a756c] ${seniorMode?"text-lg":"text-sm"}`}>먹이 {PASS}개 먹으면 통과!</p>
...
export default function SnakeGame({seniorMode,target,onComplete,onSkip}:Props){
  ...
  if(dead!==null)return(
    <div className="h-full flex flex-col items-center justify-center space-y-6 px-4">
      ...
      <p className={`text-[#c4bfb4] ${seniorMode?"text-xl":"text-base"}`}>먹이 {dead}개 (목표: {PASS}개)</p>
  ...
  return<SnakeBoard key={key} seniorMode={seniorMode} target={target} onComplete={onComplete} onSkip={onSkip} onDead={setDead}/>;
}
