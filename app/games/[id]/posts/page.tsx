...
// 변경사항:
// 1. "직접 추가" 버튼 제거
// 2. 포스트 추가는 지도 편집기에서만 가능
// 3. 좌표 없는 포스트는 목록에서 제외 (coord_x/y 있는 것만 표시)
    ...
    (async () => {
      try {
        const { data: game } = await supabase
          .from("games").select("title").eq("id", gameId).single();
        setGameTitle(game?.title ?? "");

        const data = await listPosts(supabase, gameId);
        // 좌표가 있는 포스트만 표시
        setPosts(data.filter((p) => p.coord_x !== null && p.coord_y !== null));
      } catch (e) {
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col gap-4
          ...
          <button
            ...
            className="flex items-center gap-1.5 self-start sm:self-auto
              rounded-xl border border-[#2a2924] px-4 py-2.5 text-sm text-[#7a756c]
              hover:border-[#b89a5a]/50 hover:text-[#b89a5a] transition-colors"
          >
            <MapPinIcon /> 지도에서 포스트 편집
          </button>
        </div>
        ...
        {posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-medium text-[#5a5650]">
              배치된 포스트가 없습니다
            </p>
            <p className="mt-1 text-xs text-[#3a3830]">
              지도 편집기에서 원하는 위치를 클릭해 포스트를 만드세요
            </p>
            <button
              onClick={() => router.push(`/games/${gameId}/posts/editor`)}
              className="mt-5 flex items-center gap-1.5 rounded-xl bg-[#b89a5a]
                px-5 py-2.5 text-sm font-medium text-[#0f0f10]
                hover:bg-[#c9aa6a] transition-colors"
            >
              <MapPinIcon /> 지도 편집기 열기
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="mt-8 flex flex-col-reverse gap-3
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5
                  rounded-xl border border-[#2a2924] px-5 py-2.5 text-sm
                  text-[#7a756c] hover:border-[#b89a5a]/50 hover:text-[#b89a5a]
                  transition-colors"
              >
                <MapPinIcon /> 지도 편집기로
              </button>
  ...
  return (
    <div className="rounded-xl border border-[#2a2924] bg-[#18181a] px-5 py-4
      ...
      <div className="mt-3 flex items-center gap-2 flex-wrap pl-10">
        ...
        <MetaChip>
          📍 {Number(post.coord_x).toFixed(1)}%, {Number(post.coord_y).toFixed(1)}%
        </MetaChip>
        {quizCount > 0 && post.quizzes?.map((q) => (
          <span key={q.id}
...
function StatBadge({ label, value }: { label: string; value: string | number }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1
      text-xs border border-[#2a2924] bg-[#18181a] text-[#5a5650]">
